...
@Slf4j
@Service
public class RiskPersonRiskSyncInfoServiceImpl
    extends ServiceImpl<RiskPersonRiskSyncInfoMapper, RiskPersonRiskSyncInfo>
    implements RiskPersonRiskSyncInfoService {

  @Override
  public ResultBean<Boolean> addRiskPerson(List<RiskPersonRiskSyncInfoReq> riskPersonList) {
    log.info("addRiskPerson req={}", riskPersonList);
    ...
    if (!CollectionUtils.isEmpty(riskPersonList)) {
      ...
      for (RiskPersonRiskSyncInfoReq riskPerson : riskPersonList) {
        ...
        riskPersonRiskSyncInfo.setSourceChannel(
            this.splitStrs(riskPersonRiskSyncInfo.getSourceChannel()));
        ...
        if (null != riskPerson1) {
          ...
          updateWrapper.eq(
              "risk_person_risk_sync_info_id", riskPerson1.getRiskPersonRiskSyncInfoId());
